  ...
  rtY_descr_in[784], boolean_T rtY_bits_out[784])
{
  ...
  for (i = 0; i < 128; i++) {
     ...
     */
    u += ((rtConstP.Constant_Value[i] != 0.0) == (int32_T)rtU_v2x_rx_bb_in[i]);

    /* Sum: '<S4>/Sum1' incorporates:
     *  Constant: '<S4>/Constant'
     *  Logic: '<S4>/AND1'
     */
    tmp += ((rtConstP.Constant_Value[i] != 0.0) == (int32_T)rtb_off1[i]);

    /* Sum: '<S4>/Sum2' incorporates:
     *  Constant: '<S4>/Constant'
     *  Logic: '<S4>/AND2'
     */
    tmp_0 += ((rtConstP.Constant_Value[i] != 0.0) == (int32_T)rtb_off2[i]);

    /* Sum: '<S4>/Sum3' incorporates:
     *  Constant: '<S4>/Constant'
     *  Logic: '<S4>/AND3'
     */
    tmp_1 += ((rtConstP.Constant_Value[i] != 0.0) == (int32_T)rtb_off3[i]);
  }
  ...
  for (rtb_uDMaximum = 0; rtb_uDMaximum < 88; rtb_uDMaximum++) {
    ...
    for (i = 0; i < 4; i++) {
      ...
      for (j = 6; j >= 0; j--) {
        ...
        temp = (i + 1) * j * rtConstP.RSDecoder_table2[1] % 7;
        if (temp == 0) {
          temp = 7;
        }
        ...
        for (i = 0; i < RSDecoder_OmegaZActual_idx_1 + 1; i++) {
          ...
          if (temp >= 0) {
            ...
            if ((RSDecoder_PsiZ[i] == 0) || (temp == 0)) {
              ...
              temp = (rtConstP.RSDecoder_table2[temp - 1] +
                      rtConstP.RSDecoder_table2[RSDecoder_PsiZ[i] - 1]) % 7;
              if (temp == 0) {
                temp = 7;
              }
          ...
          if (RSDecoder_OmegaZActual_idx_1 < Temp3) {
            ...
            for (i = 0; i < 5; i++) {
              ...
              if ((temp == 0) || (intVal == 0)) {
                ...
                rtDW->RSDecoder_Dz[i] = rtConstP.RSDecoder_table1[temp - 1];
              }
        ...
        while (i < 8) {
          ...
          if (RSDecoder_OmegaZActual_idx_2 == 0) {
            ...
            if (i == 0) {
              ...
              temp = rtConstP.RSDecoder_table1[6 - rtConstP.RSDecoder_table2[i -
                1]];
            }
            ...
            for (j = 0; j < noErrorStatus; j++) {
              ...
              if (i > 0) {
                ...
                if (intVal == 0) {
                  ...
                  temp = (rtConstP.RSDecoder_table2[i - 1] +
                          rtConstP.RSDecoder_table2[intVal - 1]) % 7;
                  if (temp == 0) {
                    temp = 7;
                  }
  ...
  for (j = 0; j < 784; j++) {
    ...
    rtb_uDMaximum = inv;
    for (i = 0; i < 16; i++) {
      rtb_uDMaximum -= (uint8_T)(rtConstP.Descrambler_Polynomial[i + 1] *
        shiftReg[i]);
    }
