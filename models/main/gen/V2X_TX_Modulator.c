  ...
  rtY_ps_out[67712])
{
  ...
  for (i = 0; i < 8464; i++) {
    ...
    rtY_map_out[outIdx].re = rtConstP_tx_mod.QPSKModulatorBaseband_modmap[inIdx_0];
    rtY_map_out[outIdx].im = rtConstP_tx_mod.QPSKModulatorBaseband_modmap[inIdx_0 + 1];
    outIdx++;
  }
